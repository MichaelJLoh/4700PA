...
%sets up magnetic mu dependent material grid
mu{1} = ones(nx{1},ny{1})*c_mu_0;

%sets up electric epsilon dependent material grid
epi{1} = ones(nx{1},ny{1})*c_eps_0;

for i = 1:13
    if mod(i,2)
        epi{1}(30+i*10:40+i*10,65:95)= c_eps_0*11.3;
    else
        epi{1}(30+i*10:40+i*10,55:85)= c_eps_0*11.3;
    end
end

epi{1}(40:160,20:30)= c_eps_0*11.3;
...
%sets up source boundary conditions
bc{1}.NumS = 1;
bc{1}.s(1).xpos = 20;
bc{1}.s(1).type = 'ss';
...
%sets up pml boundary absorbers
bc{1}.xm.type = 'a';
bc{1}.xp.type = 'a';
bc{1}.ym.type = 'a';
bc{1}.yp.type = 'a';
